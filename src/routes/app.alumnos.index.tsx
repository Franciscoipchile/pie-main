...
function AlumnosList() {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      setLoading(true);
      let query = supabase.from("alumnos").select("*, cursos(numero, letra), usuarios(nombre, apellido)");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4">
        ...
        {/* Aquí puedes mantener tus selectores de filtro si los necesitas */}
      </div>
          ...
          <TableBody>
            {filtrados.map((a) => (
              <TableRow key={a.id}>
                ...
                <TableCell className="font-medium">{a.cursos ? `${a.cursos.numero}° ${a.cursos.letra}` : "-"}</TableCell>

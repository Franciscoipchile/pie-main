...
async function run() {
  try {
    ...
    const { data: bloque } = await supabase.from('bloques_horarios').select('id').limit(1).single();
    const { data: asignatura } = await supabase.from('asignaturas').select('id').limit(1).single();

    if (!curso || !bloque || !asignatura) {
      console.log("Missing master data to create a test row.");
      return;
    }

    console.log("Found master data IDs:", { curso_id: curso.id, bloque_id: bloque.id, asignatura_id: asignatura.id });
    ...
    if (insertError) {
      ...
      await supabase.from('horarios').delete().eq('id', testId);
    }
